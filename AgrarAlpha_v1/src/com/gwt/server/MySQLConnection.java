...
/**
 * Class to easy handle MySQL Connections. Provides functions to connect to our
 * Cloud-SQL instance and use it in other classes.
 * 
 * @author Fabian Weber
 */
public class MySQLConnection {
	...
	Connection conn;
	public static final Logger log = Logger.getLogger(MySQLConnection.class
			.getName());
	...
	public boolean connect() {
		try {
			...
			// Checks if application is running on AppEngine or not and set up
			// corresponding url
			if (SystemProperty.environment.value() == SystemProperty.Environment.Value.Production) {
				// If application is running on AppEngine.
				Class.forName("com.mysql.jdbc.GoogleDriver");
				url = "jdbc:google:mysql://" + appengineSource + "/" + db
						+ "?user=" + user + "&charset=utf8";
				if (!password.equals(""))
	 ...
	 * 
	 * @author Fabian Weber
	 * @return <code>true</code> if connection close succeeded.
